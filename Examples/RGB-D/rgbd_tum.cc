...
int main(int argc, char **argv)
{
    ...
    for (int ni = 0; ni < vstrImageFilenamesRGB.size(); ni++) {
        ...
        getKeyPoint(featureFolder + "/" + to_string(vTimestamps[ni]) + ".png.r2d2.result/keypoints.txt", keypoints);
        local_desc.create(keypoints.size(), 128, CV_32F);
        getdescriptor(featureFolder + "/" + to_string(vTimestamps[ni]) + ".png.r2d2.result/descriptors.npy" , local_desc , keypoints.size());
        global_desc.create(512, 936, CV_32F);
        getGlobaldescriptor(featureFolder + "/glb/patchfeats_psize5_" + to_string(vTimestamps[ni]) + ".npy" , global_desc);
...
void getdescriptor(string filename,cv::Mat & descriptor,int nkeypoints){
    ...
    for(int i = 0 ; i < nkeypoints ; i ++){
        ...
        for(int j = 0; j < 128; j++){
            float temp = arr.data<float>()[i * 128 + j];
            pdata[j]= temp;
        }
    }
}
...
void getGlobaldescriptor(string filename,cv::Mat & descriptor){
    ...
    for(int i = 0; i < 512; i++){
        float* pdata = descriptor.ptr<float>(i);
        for(int j = 0; j < 936; j++){
            float temp = arr.data<float>()[i * 936 + j];
            pdata[j] = temp;
        }
    }
}
...
void getKeyPoint(string filename , vector<cv::KeyPoint> & keyPoints){
    ...
    for(int i = 0 ; i < 5000 && !getfile.eof()  ; i++)
    {
        string s;
        getline(getfile,s);
        if(!s.empty())
