...
LoopClosing::LoopClosing(Map *pMap, KeyFrameDatabase *pDB, Vocabulary *pVoc, const bool bFixScale):
...
bool LoopClosing::DetectLoop()
{
    ...
    for(size_t i=0; i<vpConnectedKeyFrames.size(); i++)
    {
        ...
        float score = CurrentBowVec.score(CurrentBowVec, BowVec);
...
bool LoopClosing::ComputeSim3()
{
    ...
    {
        database.push_back(mvpEnoughConsistentCandidates[i] -> _globaldescriptors);
    }
    int Simiindex = 0;
    ...
    // {
    //     double score = 0;
    //     for(size_t j = 0; j < database[i].rows; j++)
    //     {   
    //         double temp = 9999;
    //         cv::Mat result = database[i].row(j) * mpCurrentKF->_globaldescriptors.t();
    //         for(size_t k = 0; k < result.cols; k++)
    //         {
    //             if(result.at<double>(0, k) < temp)
    //             {
    //                 temp = result.at<double>(0, k);
    //             }
    //         }
    //         score += temp;
    //     }
    ...
    for(size_t i = 0; i < database.size(); i++)
    {
        cv::Mat result = database[i] * mpCurrentKF->_globaldescriptors.t();
        double temp = 0;
        for(size_t j = 0; j < result.rows; j++)
        {
            for(size_t k = 0; k < result.cols; k++)
            {
                temp += result.at<double>(j, k);
            }
        }
        if(temp < bestScore)
        {
    ...
    KeyFrame * best = mvpEnoughConsistentCandidates[Simiindex];
    std::vector<KeyFrame*> tempDatabase;
    tempDatabase.push_back(best);
    mvpEnoughConsistentCandidates = tempDatabase;
